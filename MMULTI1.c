...
int * rowAtValue(int rowNum, int *array) {
    ...
    int count = rowNum*4; // rows start at multiples of 4
...
int * colAtValue(int colNum, int *array) {
    ...
    int count = colNum; // columns start between 0 and 3
...
int main() {
    ...
    int shmid;
    shmid = shmget((key_t)1501, sizeof(int)*16, 0666 | IPC_CREAT);
    ...
    // This has been done this way according to instructions, in which part 1 must be hardcoded to specifically solve a 4x4 array
        ...
        if (pid2 == 0) { // it's a child
            ...
            if (pid3 == 0) { // it's a child
                ...
                else // you're at the parent
                {
                    sleep(2);
                    printf("-----------------------------------\n");
                    printf("FINAL ANSWER WITH SHARED MEMORY\n");
                    printf("-----------------------------------\n");
                    ...
                    for (int i = 0; i<16; i++) {
                        ...
                        printf("%d ", Q[i]); // FINAL ARRAY THAT IS SOLVED
                    }
                    printf("\n");
                }
            }
